        ...
        for(int i=0;i<heights.size()-1;i++){
            ...
            if(heights[i]>=heights[i+1]){
                ...
                if(ladders==0 && totalCount>bricks) return i;
                 // If count of bricks is lesses than existing bricks move to next building
                if(ladders==0 && totalCount<=bricks) continue;
                
                // Insert the current differnce into maxheap to retrive the heighest diff if needed for the ladder
                maxHeap.push(diff);
